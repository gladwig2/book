...
Now the question has become, how to select a single model from the posterior distribution and use it for future inference? What are the objectives from inference?
...
When there are correlated predictors in non-nested models, MPM in general does well. However, if the correlation among variables increases, MPM may miss important variables as the correlations tend to dilute the posterior including probabilities of related variables.  
